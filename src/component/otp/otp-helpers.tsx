...
export function otpActions(otp: string, id: string, index: number, setItems: setItemsFunction) {
  ...
  const copy = (
    <Action.CopyToClipboard title="Copy OTP" content={otp} onCopy={async () => await updateOrderIfEnabled(id, index, setItems)} />
  )

  const paste = <Action.Paste title="Output OTP" content={otp} onPaste={async () => await updateOrderIfEnabled(id, index, setItems)} />
