...
import { ensure } from "../../../../packages/shared/utils/ensure";
...
export const codeFromURIString = (id: string, uriString: string): Code => {
    try {
        ...
        if (uriString.includes("#"))
            return _codeFromURIString(id, uriString.replace(/#/g, "%23"));
        throw e;
    }
};
...
const parseSecret = (url: URL): string =>
    ensure(url.searchParams.get("secret")).replace(/ /g, "").toUpperCase();
